...
class Loader:
    ...
    def update_chunk_records_with_tx(
        ...
        Args:
            tx: The Neo4j transaction to use.
            node_type (str): The type/label of the nodes to be updated.
            records (list[dict]): A list of records to be updated.
            id_field (str, optional): The unique identifier field. Defaults to "guid".
        """
        cypher_statement = f"""
        UNWIND $records AS record
    ...
    def update_file_records(
        ...
        """
        Update records from a TSV file into the graph database in chunks.
        ATTENTION: this method only updates EXISTING nodes. NO new nodes can be created from this method.
        ...
        batch_count = 0
        if logger:
            logger.info(f"(Node Update) Start file {os.path.basename(file_path)}")
        print(f"(Node Update) Start file {os.path.basename(file_path)}")
        with self.driver.session() as tx:
            # with session.begin_transaction() as tx:
            for chunk in self.read_file_in_chunks(file_path, encoding, chunk_size):
                batch_count += 1
                ...
                result_summary = self.update_chunk_records_with_tx(
                    tx, chunk_type, records, id_field, logger=logger
                )
                # batch_end = timer()
                if logger:
                    logger.info(f"Created {result_summary['nodes_created']} nodes") # returned summary doesn't have nodes_updated key. We should expect nodes_created = 0
                    logger.info(f"Set {result_summary['properties_set']} properties")
                print(f"Created {result_summary['nodes_created']} nodes")
                print(f"Set {result_summary['properties_set']} properties")
                # print("Batch loading time (seconds): ", batch_end - batch_begin)
