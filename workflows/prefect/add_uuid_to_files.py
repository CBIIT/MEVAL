...
@flow(name="Add UUID to a set of TSV files")
def add_uuid_to_files(
    ...
    flow_logger.info(f"Found {len(tsv_file_list)} TSV files under {tsv_folder} after downloading from s3")
    ...
    for file in tsv_file_list:
        ...
        Validator.add_uuid_to_tsv_file(file_path=file,
        ...
        flow_logger.info(f"Uploaded file {output_filename} to s3 location: {output_bucket_loc}")
